...
#define kSelectionHeight    2

#import "SSImageSlider.h"

@interface SSImageSliderDefaultImageSource : NSObject <SSImageSliderImageSource>
@end

@interface SSImageSlider () <UIScrollViewDelegate>
{
    __weak id <SSImageSliderDataSource> _dataSource;
    __weak id <SSImageSliderDelegate> _delegate;
...
+ (id<SSImageSliderImageSource>)defaultDataSource {
    static SSImageSliderDefaultImageSource *_defaultDataSource = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _defaultDataSource = [SSImageSliderDefaultImageSource new];
    });
...
-(void)createIndicatorWithNumberOfElements:(float)numberOfElements {
    
    float width = _scrollView.frame.size.width / numberOfElements;
    _selectionIndicatorWidth = width;
   
    float posY  = _scrollView.frame.size.height - kSelectionHeight;
    _selectionIndicatorPositionY = posY;
    
    
    _selectionIndicatorPositionX = 0;
    
    selectionIndicator = [[UIView alloc]initWithFrame:CGRectMake(_selectionIndicatorPositionX,
                                                                 _selectionIndicatorPositionY,
                                                                 _selectionIndicatorWidth,
                                                                 kSelectionHeight)];
    
    [selectionIndicator setTranslatesAutoresizingMaskIntoConstraints:false];
    selectionIndicator.backgroundColor = _selectionColor;
    
    [self addSubview:selectionIndicator];

}

-(void)changeIndicatorPosition:(long)currentPosition animated:(BOOL)animated{
    
    [UIView animateWithDuration:0.2
                          delay:0
                        options:UIViewAnimationOptionCurveLinear
                     animations:^{
                         float newX = _selectionIndicatorWidth * currentPosition;
                         selectionIndicator.frame = CGRectMake(newX,
                                                               _selectionIndicatorPositionY,
                                                               _selectionIndicatorWidth,
                                                               kSelectionHeight);
...
- (void) loadData
{
    ...
    if([aImageUrls count] > 0) {
        numberOfElements = [aImageUrls count];
    ...
    [self createIndicatorWithNumberOfElements:numberOfElements];
}
...
- (void) scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    [self changeIndicatorPosition:currentPage animated:TRUE];
}
...
@implementation SSImageSliderDefaultImageSource
